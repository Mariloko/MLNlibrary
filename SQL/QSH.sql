...
CREATE PROCEDURE ProcessIntermediaryTable()
BEGIN
    DECLARE v_borrow_id INT;
    DECLARE v_borrow_ISBN BIGINT;

    -- Process the intermediary table while it is not empty
    WHILE EXISTS (SELECT 1 FROM intermediary_table) DO
        -- Retrieve the borrow_id and ISBN from the first row of the intermediary table
        SELECT borrow_id, ISBN INTO v_borrow_id, v_borrow_ISBN
        FROM intermediary_table
        ORDER BY borrow_id ASC
        LIMIT 1;

        -- Update the borrow table to mark the borrow as 'reserved' with the current date
        UPDATE borrow
        SET status = 'reserved', reserve_date = CURDATE()
        WHERE borrow_id = v_borrow_id;

        -- Delete the processed data from the intermediary table
        DELETE FROM intermediary_table
        WHERE borrow_id = v_borrow_id;
    END WHILE;
END //
...
BEGIN
    SELECT
        ...
        b.borrow_id,
        b.date,
        st.role AS 'Account Type',
        st.school_name AS 'School Name',
        (SELECT COUNT(*) FROM borrow WHERE username = st.username AND status IN ('reserved', 'lended', 'due return')) AS 'Borrow Count',
        (SELECT MAX(CASE WHEN status = 'due return' THEN 'Yes' ELSE 'No' END) FROM borrow WHERE username = st.username) AS 'Due Return Status'
    FROM
        pending_borrows AS b
        INNER JOIN students_teachers AS st ON b.username = st.username
    WHERE
        st.school_name = p_school_name
    ORDER BY
        b.date DESC;
END //
...
CREATE PROCEDURE AddBorrowsToQueue(IN p_ISBN BIGINT, IN p_school_name VARCHAR(50), IN p_num_borrows INT)
BEGIN
    DECLARE v_counter INT DEFAULT 0;
    DECLARE v_count_in_queue INT;
    
    -- Check the count of 'in queue' borrows for the book
    SELECT COUNT(*) INTO v_count_in_queue
    FROM borrow b 
    JOIN book_school_unit bc ON b.ISBN = bc.ISBN 
    WHERE b.status = 'in queue' AND b.ISBN = p_ISBN AND bc.school_name = p_school_name;
    
    -- Determine the number of borrows to add based on p_num_borrows and count_in_queue
    SET v_counter = LEAST(p_num_borrows, v_count_in_queue);
    
    -- Add 'in queue' borrows to the intermediary table
    WHILE v_counter > 0 DO
        -- Retrieve the next 'in queue' borrow_id for the book
        SELECT borrow_id
        INTO @v_borrow_id
        FROM borrow
        WHERE status = 'in queue' AND ISBN = p_ISBN
        AND borrow_id > IFNULL((SELECT MAX(borrow_id) FROM intermediary_table), 0)
        ORDER BY borrow_id ASC
        LIMIT 1;
        
        IF @v_borrow_id IS NOT NULL THEN
            -- Insert the borrow_id and ISBN into the intermediary table
            INSERT INTO intermediary_table (borrow_id, ISBN)
            VALUES (@v_borrow_id, p_ISBN);
...
IN p_new_copies INT)
BEGIN
CALL AddBorrowsToQueue(p_ISBN, p_school, p_new_copies);
UPDATE book_school_unit
...
BEGIN
  ...
  IF v_exists = 0 THEN
    SELECT 'Invalid school name' AS result;
  ELSEIF p_manager_key <> 4557370293 THEN
    SELECT 'Wrong key' AS result;
  ELSE
...
CREATE PROCEDURE GetUserDetails(IN p_username VARCHAR(255))
BEGIN
    ...
    FROM users u
    INNER JOIN students_teachers st ON u.username = st.username
    WHERE u.username = p_username OR p_username IS NULL;
END //
